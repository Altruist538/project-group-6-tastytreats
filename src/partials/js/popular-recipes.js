...
// import { measureRating, ratingScale } from '../renders/renders';
import SmoothScrollbar from 'smooth-scrollbar';
import Notiflix from 'notiflix';
...
function addData(markup) {
  refs.modalRecipes.insertAdjacentHTML('beforeend', markup);
}
...
async function genereteRecipe(id) {
  try {
    ...
    addData(CreateMarkup(recipe));
    addScrollbarText();
    // addRatingButton(); // Якщо ця функція має бути використана, її потрібно додати та імпортувати
  } catch (err) {
    console.error(err);
  }
}
